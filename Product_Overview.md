...
- **个人记忆库**：AI 记住你的研究画像与写作偏好，可随时查看、修改、删除；日常交流中自动沉淀的新偏好若与旧记忆冲突，会弹出裁决卡请你确认。价值：记忆透明可控，系统随使用越用越懂你。

![图 2：个人记忆库（高置信记忆 + 冲突裁决）](./screenshots/02-memory.png)

- **研究图景思维导图**：以思维导图呈现领域知识全景，每个节点含进展解读与研究缺口。价值：快速掌握领域现状，找到可切入的研究空白点。

![图 3：研究图景思维导图（节点含进展解读与研究缺口）](./screenshots/03-landscape.png)

### 模块二 · 文献筛选矩阵 —— 几十篇文献几秒内看清

- **文献筛选矩阵**：上传文献后 AI 自动从质量、相关性、方法论、创新性四个维度打分并直显评分理由，按总分排序、低分自动折叠、权重可调。价值：几十篇文献几秒内排出阅读优先级，先读哪篇一目了然。
...
- **三栏沉浸精读**：左侧一页纸预读卡讲清文献的核心问题、方法、结论与局限，先判断值不值得细读；中栏原文划线打标（重点论据/借鉴方法/存疑/背景），批注自动绑定原文片段并成为写作时优先引用的素材；右栏伴读问答基于本文档证据回答，每个答案都能一键跳回原文对应位置核实。价值：读得快、记得住、问得清，读过的内容写论文时都用得上。

![图 6：三栏沉浸精读（预读卡 + PDF 原文划选批注 + 伴读问答溯源）](./screenshots/06-reader.png)

### 模块四 · 多模式写作工作台 —— 写作又快又可控

- **写作提案卡**：AI 每步只提修改建议（追加/修改/删除，含改动说明），你确认后才写入编辑器，全程留快照可回退；文内引用三色校验（正常/存疑/无效），点击引用角标即可定位原文。价值：AI 帮你写但不替你写，修改始终可控、可溯源。
