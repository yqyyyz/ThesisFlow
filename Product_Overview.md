...
| 轮次 | 主题 | 一句话成果 |
|------|------|-----------|
| 1 | 核心功能搭建 | 四大模块完整闭环 + 上下文工程骨架（三层 Prompt / 三档权重路由 / 引用溯源） |
...
| 5 | P0+P1 架构升级 | 意图路由、二层缓存、Parent-Child 切片、Lazy GraphRAG、引用校验升级、观测端点——成本-质量平衡体系 |
...
贯穿全程的产品决策思路：**成本-质量分级**（强弱模型分槽调用）、**引用溯源设计**（APA 引用·点击定位原文）、**记忆生命周期**（置信度升降 + 冲突裁决 + 健康报告）、**数据埋点支撑 KPI**（引用质量、路由分布、缓存命中全量落表）。
