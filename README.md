...
| 本文 README.md | 工程文档：问题背景、功能概览、快速启动与演示动线、项目结构 |
...
- **进入**：侧边栏左上角「🎬 演示」按钮 → 出现浮动引导面板（六阶段操作指引 + 一键跳转/复制指令）；台词卡与逐阶段技术动线见 `ThesisFlow_Demo_Technical_Flow.md`；
...
6. 「写作工作台」起草模式：与 AI 商讨后点「同步到大纲」（Markdown 大纲，可预览/编辑）；切写作模式：右侧对话输入指令（续写/修改/删除）→ 审核提案卡（三种模式徽标 + APA 引用）→ 采纳写入/拒绝继续讨论；
