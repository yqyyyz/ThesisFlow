...
> **Demo 范围说明**：本文档描述 ThesisFlow 的完整产品方案。当前 Demo 覆盖领域知识库、文献筛选矩阵、沉浸式精读、多模式写作工作台，重点验证“筛选—精读—协作写作”主流程中的决策质量、证据可信度与操作可控性。长期记忆作为持续使用后的附加价值，多人协作与移动端适配属于后续扩展。
>
...
## 一、 产品定位与核心价值
*   **一句话定位**：ThesisFlow 是一款面向科研任务的 AI 文献与写作工作台，支持文献筛选、文献精读和高效人机协作，通过复用阅读成果与积累研究上下文，减少从文献整理到综述写作的重复劳动。
*   **适用范围**：适用于需要围绕明确研究问题完成文献筛选、证据阅读与带引用写作的高校师生和研究人员，不限定单一学科或身份阶段。
*   **核心产品假设**：在同一任务空间中连接评分、精读批注、证据检索和提案式写作，可以降低重复查找与修改失控；长期记忆的真实收益需在重复任务中另行验证。

| 场景 | 核心问题 | 产品价值 |
|---|---|---|
| 文献筛选 | 文献较多，难以确定阅读顺序 | 四维评分提供结构化比较，AI 精读建议结合研究问题、筛选标准和文献内容给出阅读优先级与理由 |
| 文献精读 | 理解耗时，笔记与原文分离，阅读成果难复用 | 预读卡给出阅读重点，伴读回答可回到原文，语义明确的批注可进入写作上下文 |
| 高效人机协作 | AI 修改范围难控制，引用需要逐一核查 | 提案采纳后才写入；引用分别检查来源、定位和证据支持；快照支持回退 |
| 跨任务积累 | 背景与偏好需要重复交代 | 可控记忆复用稳定偏好与背景，但当前指令优先，且不将采纳行为等同于内容正确 |

## 二、 核心任务场景与任务流程 (Core Tasks & Flow)

### 2.1 主流程

1. **明确研究问题**：创建项目并填写研究问题；筛选标准可同时填写，也可稍后在项目设置中补充。
2. **导入与筛选**：直接上传 PDF、Word 或 Markdown。系统展示四维评分、缺失信息、AI 精读建议与理由；用户可关闭自动折叠并人工校正评分。
3. **精读与批注**：进入单篇精读，先查看核心问题、方法、结论、局限与阅读重点，再通过伴读问答和原文定位核查；批注按用途标记。
4. **协作写作与审查**：选择批注素材，AI 生成追加、替换或删除提案。提案未采纳前不改正文；过期或定位失败时保留提案与用户内容。
5. **导出和继续修改**：导出 Word、LaTeX 或 Markdown；写作过程自动保存快照，可在修改失败或方向变化时回退。

### 2.2 首次使用与可选能力

*   用户可仅填写项目名称后立即进入项目并导入文献，身份设定、领域知识积累和长期记忆均不是首次使用的前置条件。
*   未填写研究问题或文献内容不足时，系统明确显示“信息不足”，不以默认分数或虚构元数据补齐判断。
*   领域知识库和长期记忆为跨任务复用入口，不阻断项目内筛选、精读和写作主流程。

## 三、 竞品分析与差异化定位 (Competitive Landscape)

| 竞品 | 核心定位 | ThesisFlow 的差异化优势 |
|------|---------|----------------------|
| **Elicit** | 文献发现、筛选与研究报告 | ThesisFlow 当前更强调项目内原文精读、批注复用和提案式写作衔接 |
| **Scite** | 引用上下文与来源核查 | ThesisFlow 将来源存在、原文定位和证据支持检查放到写作提案采纳前 |
| **Notion AI** | 通用知识管理与 AI 写作 | ThesisFlow 以研究问题、筛选标准、文献片段和批注标签组织科研任务上下文 |
| **Overleaf + AI 能力** | LaTeX 协作与论文写作 | ThesisFlow 当前侧重写作前的筛选、精读成果复用和可回退的提案交互 |
| **Consensus / Semantic Scholar** | 学术搜索与发现 | ThesisFlow 的重点是导入后的阅读决策、原文核查和写作协作 |

**差异化总结**：ThesisFlow 不以“功能唯一”作为主张，而是验证筛选、精读和协作写作能否在同一项目上下文中减少重复查找、核验与返工。

## 四、 全局信息架构与空间划分 (Information Architecture)
为了保证复杂科研任务的有序性，系统采用“可选个人设置 + 可选知识管理域 + 独立项目空间”的分层设计，项目空间是首次使用和主任务的直接入口。

### 1. 可选个性化设置 (Optional Personalization)
*   **非前置设置**：用户无需先完成人设或领域初始化，可直接创建项目、填写研究问题并导入文献。
*   **偏好设置**：引用规范、写作语言和稳定写作偏好可用于后续请求；当前项目内明确指令始终优先于历史偏好。
...
*   **交互细节**：
    *   **领域图景生成 (Landscape Overview)**：AI 按需基于当前资料生成宏观报告与横向思维导图。`is_gap` 节点仅表示当前资料范围内有依据的候选研究问题，不代表学界空白；材料不足时允许不生成候选问题或返回空分支。
...
*   **交互细节**：
    ...
    *   **四维评分与 AI 精读建议**：导入后展示质量、相关性、方法论严谨度、创新性四维评分，并另行输出“推荐精读 / 暂不优先 / 信息不足”及具体理由。精读建议综合研究问题、筛选标准和文献内容，不由加权总分阈值直接决定。
        *   **评分标准**：每维度 1-5 分；无法判断时为 `null` 并说明缺少的信息。**质量**——仅依据已有发表载体、被引量和写作规范性，不补猜缺失元数据；**相关性**——与当前项目研究问题的语义匹配度；**方法论严谨度**——实验设计、样本与分析是否完整；**创新性**——是否提出新理论、新方法或新数据。学科锚点只校准判据，不替代文献证据。
    ...
    *   **加权排序与折叠**：各维度权重可调，缺失维度不以默认分参与计算。仅“暂不优先”且加权总分低于阈值的就绪文献自动折叠；“推荐精读”和“信息不足”不自动隐藏，用户可随时关闭折叠。
...
*   **交互细节**：
    *   **AI 预读与评价**：侧边栏生成结构化预读卡，包含核心问题、方法、结论、贡献、局限和阅读重点；阅读重点指出需要回到原文核对的方法、结果或限制。
    *   **原文划选批注**：PDF、Word 和 Markdown 均支持划选打标、划线与备注。标签带有明确用途：“重点论据”是候选支持证据，仍需核验；“借鉴方法”仅用于方法参考；“存疑”用于争议、限制和待核查内容；“背景”用于定义与背景说明。批注绑定原文 Chunk 并提升检索优先级，但不改变其证据性质。
...
*   **交互细节**：
    ...
    *   **状态 2：写作模式 (Writing Mode)**：用户输入指令后，AI 生成追加、替换或删除提案；提案分别展示来源是否存在、原文能否定位、证据是否支持主张。用户采纳后才执行锚点定位与写入校验，拒绝不改正文；文稿变化会使旧提案过期，定位或写入失败时保留正文、用户指令和提案内容供重试。引用按作者—年份显示，点击后定位原文片段。
    ...
    *   **版本历史与回退**：系统在正文更新前保存旧内容，支持按时间线浏览和回退；回退前再次保存当前内容，避免回退操作覆盖唯一可恢复版本。
    ...
    *   **用户反馈记录**：采纳、拒绝和编辑先作为行为记录，不直接等同于内容正确、来源可靠或文献质量。后续来源降权必须结合可选反馈原因与内容检查，不能因表达方式不符合偏好而降低文献权重。
...
*   **分层校验与状态标注**：引用不再用一个“正常”状态概括全部正确性。系统分别检查文献来源是否存在、Chunk 是否能映射到对应文献，以及证据对当前主张是支持、存疑、不支持还是冲突；结构检查失败时不继续暗示已完成语义支持判断。
*   **前端逆向映射**：前端将角标渲染为作者—年份格式，并在提案卡并列展示三层校验结果。点击引用后通过 `chunk_id` 跳转精读页并定位原文；无法定位时明确显示失败，而不是以来源存在代替定位正确。
...
*   **隐式记忆生命周期**：起草商讨每满 4 轮提取候选稳定偏好，不沉淀一次性任务指令。记忆可查看、编辑、删除并带置信度与时效性；冲突或长期未使用时由用户裁决。当前明确指令优先于历史偏好，项目事实不得从其他项目记忆直接迁入；研究问题变化后，相关建议与记忆需重新核查。
...
## 八、 评测目标与指标 (Evaluation)

Demo 阶段先建立可复现基线，不预设提效结论。指标按首次失败环节记录，并报告成功数、分母与局限。

| 场景 | 核心指标 |
|---|---|
| 文献筛选 | 重要文献召回、推荐准确性、理由可靠性、筛选耗时、人工纠正与盲从情况 |
| 文献精读 | 回答正确性、原文定位、阅读重点有用性、任务耗时、关键限制遗漏 |
| 协作写作 | 批注用途保持、主张—证据对应、提案范围、采纳/拒绝/回退可靠性、核验与返工成本 |
| 长期记忆 | 重复交代次数、错误使用、项目隔离、更新删除；真实收益仅通过后续重复任务观察 |

P1 正式测评使用 30 个核心任务（筛选/精读/协作写作各 10 个）和 8 项记忆/反馈回归。每次模型调用同时记录实际 System Prompt、User Prompt、完整 messages、RAG/记忆/批注上下文、裁剪决策、模型参数与原始输出；没有 System Prompt 的调用显式记为 `none`。详细协议见 `ThesisFlow_P1_产品测评方案.md`，可执行资产见 `evaluation/p1/`。
...
- **消费侧**：维度分写入 `documents.scores`，精读建议独立写入 `documents.reading_recommendation`；缺失分数不参与加权。矩阵按加权总分排序，但仅将“暂不优先”且低于阈值的文献标记为可折叠。
...
3. **生成后分层校验**：每个引用标记分别输出 `source_status`、`location_status`、`support_status`；先检查文献和 Chunk 映射，再进行向量相似度与必要的 NLI 判断。提案卡分别展示三层结果，综合状态仅用于编辑器颜色兼容。
...
- **反馈记录**：用户对 AI 内容的采纳/拒绝/修改写入 `ai_feedback`。P0 仅把它作为行为记录，不自动修改来源权重；后续若用于检索调整，必须增加拒绝原因、内容检查和可回归验证。
...
- **领域图景（报告 + 思维导图）**：材料装配 = 近 15 篇文献（KB + 项目，各带 ≤150 字摘要）+ 活跃记忆 Top6 + 用户领域；报告在材料不足处如实标注。思维导图节点包含 `label + detail + related_doc_ids + is_gap`，其中 `is_gap` 表示当前资料支持的候选研究问题，不强制数量，资料不足时允许空分支。
...
| POST | `/api/admin/demo-reset` | 种子数据秒级复位（快照恢复，不重跑 LLM） |
...
- **成本控制**：强弱模型分级（写作/审查走强模型，评分/摘要/记忆提取走轻量模型）；单项目 token 用量看板；批量化 embed 与 rerank；二层缓存（9.7.8）与签名缓存（预读卡/进展摘要/图谱）将重复 LLM 调用降到零；种子数据全流程成本在个位数元级。
...
- **种子数据**：小林画像（AI 博士生·上下文工程与长上下文 RAG·APA·中文）+ 10 条高置信记忆 + 演示项目《AI 长上下文管理的工程化措施综述与展望》+ 24 篇真实 arXiv 论文（四大主题域 + 4 篇低相关样本演示自动折叠，PDF/Word/Markdown 三格式）+ 预生成脉络图谱；快照机制 = `data/demo_fixture.json`（含向量）+ uploads 快照，`POST /api/admin/demo-reset` 秒级恢复（纯数据恢复，不重跑 LLM），前端入口为「个人设置」弹窗底部「重置演示数据」按钮。
...
| 观测 | `backend/app/core/llm.py` / `backend/app/api/observability.py` | LLM/路由/缓存/校验口径聚合；System/User Prompt、messages、上下文和原始输出追踪与 JSONL 导出 |
...
| **引用校验升级** | 分层状态 | `citations` 保留综合状态用于兼容与观测；提案响应另外输出来源、定位和证据支持三层结果 |
...
### 10.7 第八轮：评分反馈学习与写作/审查精度修复（6 项反馈）
...
### 10.10 P0：主流程决策与可信度升级

| 优化项 | 当前实现 |
|---|---|
| 定位与首次使用 | PRD 改为任务场景与主流程；项目可直接创建，研究问题与筛选标准均可后补 |
| 筛选决策 | 四维评分与 AI 精读建议分离；缺失分数为 `null`；仅“暂不优先”的低分文献折叠且可关闭 |
| 精读复用 | 预读卡增加阅读重点；四类批注用途在精读和写作界面明确展示并写入生成约束 |
| 引用可信度 | 提案卡分别展示来源存在、定位正确和证据支持状态，不再用单一“正常”暗示全面正确 |
| 协作可靠性 | 历史或正文变化后的提案标记过期；定位/写入失败保留提案；请求失败恢复用户输入；正文更新前保存快照，回退前再次备份 |
| 研究图景与记忆 | 候选研究问题允许为空且限定当前资料范围；当前指令优先，采纳/拒绝仅记录行为 |
...
### 10.12 P1 测评启动与 Prompt 可追溯

- 建立 `evaluation/p1/` 执行包：24 篇文献清单、30 张核心任务卡、S01/R01/W01 试评标准、Prompt registry 与运行/评分/失败记录模板。
- `llm.py` 对每次生成式调用记录 Prompt call ID、环节、模板 ID/版本、渲染后 System/User Prompt、完整 messages、上下文清单、模型配置、哈希、原始/解析输出与错误。
- API 请求可通过 `X-Evaluation-Run-Id` / `X-Evaluation-Task-Id` 关联运行；`GET /api/observability/prompt-calls` 按运行、任务或环节导出 JSONL，支持隐去敏感内容的元数据导出。
- Prompt 日志写盘前对 API Key/授权字段脱敏；原始日志仅作本地受控测评，不直接用于对外展示。
