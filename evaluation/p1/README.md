...
- 「人工审核」页面直接显示预先运行得到的模型输出、实际 Prompt 与核对依据；按提示完成判断并保存，不需要复制粘贴。
...
1. 启动 ThesisFlow 后端和独立工作台。
2. 由 Codex 或测评程序运行当前待检查项目，使输出关联任务编号并进入本地追踪记录。
3. 在工作台直接阅读模型输出与核对依据，点击核对项、填写问题和正确结论，保存到本地。
